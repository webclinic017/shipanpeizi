...
class ChartPainter extends BaseChartPainter {
  ...
  void drawDate(Canvas canvas, Size size) {
    ...
    for (var i = 0; i <= ChartStyle.gridColumns; ++i) {
      ...
      if (translateX >= startX && translateX <= stopX) {
        ...
        if(i == 0){
          tp.paint(canvas, Offset((columnSpace * i - tp.width / 2)+15, y));
        }else{
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    TextPainter dateTp = getTextPainter(getDate(point.detail_time),color: Colors.white);
  ...
  void drawRealTimePrice(Canvas canvas, Size size) {
    ...
    if (tp.width < max) {
      ...
      if (isLine) {
        ...
        pointPaint.shader = pointGradient.createShader(Rect.fromCircle(center: Offset(x, y), radius: 6.0));
        canvas.drawCircle(Offset(x, y), 6.0, pointPaint);
        canvas.drawCircle(Offset(x, y), 3.0, realTimePaint..color = Colors.white);
      } else {
        stopAnimation(); //停止一闪闪
      }
  ...
  String getDate(String date) => date;
